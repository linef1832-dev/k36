...
function initCalculator() {
    ...
    // ==========================================
    // ➕ ฟังก์ชันเพิ่มเว็บใหม่
    // ==========================================
    async function addNewCalcTeam() {
        // เพิ่มความปลอดภัย: เช็คสิทธิ์อีกครั้งก่อนทำงาน
        if (typeof currentUser !== 'undefined' && currentUser.role !== 'manager' && currentUser.role !== 'admin') {
             return; // หยุดทำงานถ้าไม่ใช่ admin/manager
        }

        const { value: newTeam } = await Swal.fire({
            title: 'เพิ่มเว็บใหม่',
            input: 'text',
            inputLabel: 'ชื่อเว็บ/ทีม',
        ...
        if (newTeam) {
            const select = document.getElementById('calcTeamSelect');
            let exists = false;
            for(let i=0; i<select.options.length; i++) { if(select.options[i].value === newTeam) exists = true; }
            ...
            if(!exists) {
                ...
                select.value = newTeam;

                if(typeof TEAM_LIST !== 'undefined') TEAM_LIST.push(newTeam);
                if(typeof appDB !== 'undefined') await appDB.from('settings').upsert([{ key: `calc_deduct_${newTeam}`, value: 0 }]);
                
                saveCurrentTeamsToLocal(); 
                loadCalcSettings();
    ...
    async function deleteCalcTeam() {
        ...
        if (select.options.length <= 1) {
            Swal.fire({ icon: 'warning', title: 'ลบไม่ได้', text: 'ต้องเหลืออย่างน้อย 1 รายการ', toast: true, timer: 2000 });
            return;
        }

        const confirm = await Swal.fire({
            title: `ลบ "${selectedValue}" ไหม?`,
            text: "รายการนี้จะหายไปจากเครื่องนี้ถาวร",
            icon: 'warning',
            showCancelButton: true,
        ...
        if (confirm.isConfirmed) {
            ...
            saveCurrentTeamsToLocal();
            loadCalcSettings(); 
            Swal.fire({ icon: 'success', title: 'ลบเรียบร้อย', toast: true, position: 'top-end', showConfirmButton: false, timer: 1000 });
        }
    }

    function saveCurrentTeamsToLocal() {
        const select = document.getElementById('calcTeamSelect');
        const currentOptions = Array.from(select.options).map(o => o.value);
        localStorage.setItem(CALC_STORAGE_KEY, JSON.stringify(currentOptions));
    }

    // ==========================================
    // 🔄 Override showPage
    // ==========================================
    const _originalShowPageForSheet = window.showPage;
    window.showPage = function(page) {
        if(typeof _originalShowPageForSheet === 'function') _originalShowPageForSheet(page);
        if (page === 'sheet') {
